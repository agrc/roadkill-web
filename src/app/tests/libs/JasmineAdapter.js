/*global jasmine, Reporter, playback, jstestdriver, TestCase, window, console*/
/**
...
(function() {
	...
	function intercept(method) {
		var bucket = intercepted[method] = [], meth = window[method];
	...
	function playback() {
		for(var method in intercepted) {
			if(intercepted.hasOwnProperty(method)) {
	...
	Reporter.prototype.reportSpecResults = function(spec) {
		...
		this.onTestDone(new jstestdriver.TestResult(spec.suite.getFullName(), spec.description, state, jstestdriver.angular.toJson(messages), this.specLog.join('\n'), elapsed));
	};
	...
	jstestdriver.pluginRegistrar.register({
		...
		getTestRunsConfigurationFor: function(testCaseInfos, expressions, testRunsConfiguration) {
			console.log('jasmineAdapter.getTestRunsConfiguration');
			...
			return false;
			// allow other TestCases to be collected.
		},
		runTestConfiguration: function(config, onTestDone, onComplete) {
			console.log('jasmineAdapter.runTestConfiguration');
		...
		onTestsFinish: function() {
			console.log('jasmineAdapter.onTestsFinish');
			jasmine.currentEnv_ = null;
			collectMode = true;
		}
		// loadSource: function() {
			// console.log('jasmineAdapter.loadSource');
			// return true;
		// }
	});
